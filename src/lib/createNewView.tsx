...
export async function createNewView({ html, style, viewId, id, type, resize }: { html: string, style: string, viewId: string, id?: string, type: "design" | "widget", resize?: boolean }) {
  ...
  root.render(
    React.createElement(
      NewView,
      { html, id, doc, type, resize },
      null
    )
  );
}

function NewView({ html, doc, id, type, resize }: { html: string, doc: Document, id?: string | string[], type: "design" | "widget", resize?: boolean }): JSX.Element {
